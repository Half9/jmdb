<template>
  ...
  <Splide
v-if="showSearch" aria-labelledby="Quick Search Results" :options="{
    fixedWidth: '150px',
    pagination: false,
    drag: 'free',
    snap: true,
    ...
    <SplideSlide v-for="(item, id) in searchResult.slice(0, 14)" :key="id" class="card flex flex-col">
      <router-link :to="'/' + item.media_type + '/' + item.id">
        <div class="poster">
          <img
v-if="item.media_type !== 'person' && item.poster_path !== null" :src="`${picUrl}${item.poster_path}`"
            :alt="item.title" />
          <img
v-else-if="
            item.media_type == 'person' && item.profile_path !== null
          " :src="`${picUrl}${item.profile_path}`" :alt="item.title" />
        ...
        <div class="info">
          ...
          <p>
            ...
            }}</span><span v-if="item.first_air_date">{{
